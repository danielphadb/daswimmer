...
1. **Preloader (Build-up):** Ivory-Fläche, Meta-Zeile oben (Claim links, Ort rechts), Wortmarke mittig-links; darunter faint Hairline in voller Breite, über die eine dunkle Linie als Fortschrittsbalken synchron zum Zähler mitwächst; rechts unten riesiger kursiver Serif-Zähler (0 bis 100 %). Dauer max 2,5s, danach Curtain-Reveal nach oben.
2. **Split-Hero:** zwei Spalten (ca. 64/36). Links oben große Serif-Headline (eine Zeile) mit kleiner Grotesk-Subline; links unten kleineres Bild (4:3) neben Stat-Block (große Serif-Zahl + Label) und dunklem Pfeil-Button. Rechts ein Bild in voller Höhe der Hero. Beim Scroll folgen Mikro-Label + großer Intro-Absatz, dann Full-width-Bild mit Clip-Reveal. Referenz-Implementierung: `hero_das-wimmer.html`.
